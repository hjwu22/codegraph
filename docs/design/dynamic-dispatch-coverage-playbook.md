...
Status legend: ✅ done+validated · 🟡 deterministic coverage complete, paid agent A/B pending explicit authorization · 🔬 hole identified · ⬜ not started.
...
| Android/AOSP mixed languages | JNI; AIDL/Binder; Device Tree driver/schema; init/build; VINTF/HAL; SELinux/runtime | Java/Kotlin native→C/C++; AIDL interface/method→binding implementation; DTS compatible→driver probe/schema; init service→build target; VINTF instance→AIDL/HIDL; policy context→service/property | S + X | 🟡 **Deterministic graph coverage complete:** `aosp-synthesis.test.ts` exercises every channel with exact evidence, overload/near-miss controls, and a one-read-per-type Binder scaling assertion; `aosp-end-to-end.test.ts` persists JNI/init/build/test-impact edges through a real temporary `indexAll()` and built CLI. Extraction smoke, bounded to AOSP artifacts: `platform_system_core` 218 files / 1,054 nodes / 2,508 refs / 0 warnings; `platform_build` 407 / 1,599 / 1,988 / 0; `platform_frameworks_base` 2,500-file cap / 17,252 / 17,927 / 0. This is **not** an agent A/B: the required Sonnet/high ≥2 runs/arm over S/M/L repos and ≥3 prompts remains pending explicit authorization for the paid external action. Full evidence and residuals: `docs/achievements/aosp-support.md` and `docs/reviews/aosp-review-remediation.md`. |
