...
| 5 | Unresolved-reference metadata was not persisted | Fixed end-to-end: typed metadata, schema column, schema v10 migration, single/batch inserts, every read path, static provider, and a DB batch round-trip test. Extraction version 27 forces re-indexing because a migration cannot reconstruct prior variant evidence. (Later moved to 28 by the AIDL comment-masking fix on this branch; both are unreleased, so they ship as one bump.) |
