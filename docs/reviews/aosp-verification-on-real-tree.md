...
| 檔案數 | 節點 | 邊 | DB | 索引時間 | `cFnPtr` 子步驟 E |
|---:|---:|---:|---:|---:|---:|
| 22,830 | 327,636 | 677,612 | 753 MB | 8m11s | 300,739 ms |
| 28,707 | 555,686 | 1,284,602 | **1.3 GB** | **16m37s** | **692,476 ms** |

檔案 ×1.26 → 節點 ×1.70 → 時間 ×2.03。

**峰值 RSS 實測 5,276 MB**(28,707 檔那次,每 5 秒取樣 `/proc/<pid>/status` 的 `VmRSS`)。
只有這一個峰值數據點 —— 較小那次量到的 2.9 GB 是中途取樣,不是峰值,兩者不可比。

### 時間的超線性來自既有元件

`cFnPtr` 子步驟 E 的內部計數器幾乎沒動(`read` 8,396→8,820、`nodesInFile` 11,540 vs
10,992,約 +5%),耗時卻 ×2.30。它不隨檔案數成長,而是隨符號規模超線性成長(約
nodes^1.6)。在較大那次索引中,`cFnPtrEdges` 佔 732s / 總計 846s = **86% 的 resolution 時間**。

這是**既有的 C 函式指標 synthesizer,不屬於這個 PR**,但它會主導任何平台層規模的嘗試。

### 外推

以每節點 ~9.7 KB 峰值 RSS(扣除約 1 GB 固定開銷後約 7.5 KB/節點)、每節點 ~2.4 KB DB、
每檔 ~19.4 節點計算:

| 目標 | 檔案 | 推估節點 | 推估 DB | 推估峰值 RSS | 本機 30 GB RAM |
|---|---:|---:|---:|---:|---|
| 已實測 | 28,707 | 556k | 1.3 GB | **5.2 GB(實測)** | ✅ |
| 平台層 | 209k | ~4.0M | ~10 GB | **~31 GB** | ❌ 臨界/超出 |
| 全部原始檔 | 846k | ~16.4M | ~39 GB | **~124 GB** | ❌ 硬性失敗 |

**這台機器的可行上限約 17 萬檔**(留 4 GB 給 OS:25 GB ÷ 7.5 KB/節點 ≈ 3.3M 節點 ÷ 19.4
節點/檔)。那是 AOSP 原始檔的 20%、平台層的 81%。

外推假設 RSS 隨節點數線性成長。這對 synthesizer 把節點陣列整批 materialize 的行為是合理
的,但 `cFnPtr` 的超線性時間可能伴隨超線性記憶體 —— 若如此,上限會比 17 萬更低。
...
`resolution/types.ts:79` 說明 `iterateNodesByKind` 存在的目的正是避免撐大 per-kind 陣列快取,
但這裡拿到 iterator 後立刻 spread 成完整陣列;7 個 synthesizer 各呼叫一到三次。

### 分開索引子樹反而更有效

kernel 那 6,782 檔只花 **29 秒**就跑完,並產出 11,456 條正確的 driver 綁定邊。本文三個
channel 的驗證全部是靠「挑對子樹」達成的,不是靠索引全部。這對 A/B 的 scope 設計也成立。
